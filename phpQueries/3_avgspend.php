<?php
    ...
    $query = "SELECT round(round(sum(priceInEUR),2)/count(distinct accountName),2) as avgspend
FROM sales
WHERE YEAR(saleDateTime)=2015 AND MONTH(saleDateTime)=4";
	$query_m = "SELECT round(round(sum(priceInEUR),2)/count(distinct accountName),2) as avgm
FROM sales
